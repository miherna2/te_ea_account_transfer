...
def load_inventory(path):
    ...
    with path.open(newline='', encoding='utf-8-sig') as handle:
        ...
        if reader.fieldnames != EXPECTED_COLUMNS:
            raise ConfigurationError('Inventory header must be exactly: hostname,ip_address (no credential columns)')
        agents = []
        seen_hosts = set()
        seen_ips = set()
        for line_number, row in enumerate(reader, start=2):
            ...
            try:
                ...
                raise ConfigurationError(f'Inventory line {line_number} has invalid IP address: {raw_ip}') from exc
            host_key = hostname.casefold()
            if host_key in seen_hosts or ip_address in seen_ips:
                raise ConfigurationError(f'Inventory line {line_number} duplicates a hostname or IP address')
