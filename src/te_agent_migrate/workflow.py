...
def test_agent_ids(test: TestRecord) -> set[str]:
...
class MigrationRunner:
    def __init__(
        self,
        *,
        api: ThousandEyesApiClient,
        ui_factory: UiFactory,
        report: RunReport,
        prompter: OperatorPrompter,
        console: MigrationConsole,
        options: RunOptions,
        source_group: AccountGroup,
        target_group: AccountGroup,
        target_account_token: str,
    ) -> None:
        self.api = api
    ...
    def step(
        ...
        *,
        ...
        try:
            ...
            event = StepEvent(
                utc_now(), correlation_id, agent, name, StepStatus.FAILED, duration, str(exc)
            )
    ...
    def _event(
        ...
        self.report.record_step(
            StepEvent(utc_now(), correlation_id, agent, step, status, 0.0, detail, data or {})
        )
        self.console.status(status, agent or "global", step, detail)

    def run(self, inventory: list[InventoryAgent]) -> RunReport:
        self.report.metadata.update(
            {
                "source_aid": self.source_group.aid,
                "source_account_group": self.source_group.name,
                "target_aid": self.target_group.aid,
                "target_account_group": self.target_group.name,
        ...
        batches = [
            inventory[index : index + self.options.parallelism]
            for index in range(0, len(inventory), self.options.parallelism)
        ]
        for batch_index, batch in enumerate(batches, start=1):
            self.console.info(
                "run",
                f"starting batch {batch_index}/{len(batches)}: "
                + ", ".join(agent.hostname for agent in batch),
            )
                    ...
                    self._recap.append(
                        {
                            ...
                            "status": "ok",
                            "detail": self.options.mode.value,
                        }
                    )
                    continue
            ...
            hard_stop = next(
                (error for _, _, error in outcomes if isinstance(error, HardStop)), None
            )
            ...
            for inventory_agent, correlation_id, error in outcomes:
                ...
                if not self._pause_after_failure(inventory_agent, error, correlation_id):
                    ...
                    raise HumanDecisionRequired(
                        f"Operator aborted after failure on {inventory_agent.hostname}"
                    ) from error
            if batch_index < len(batches) and not self.prompter.continue_after_batch(
                batch_index, [agent.hostname for agent in batch]
            ):
                self._event(
                    str(uuid.uuid4()),
                    None,
                    "batch_pause",
                    StepStatus.PAUSED,
                    f"Operator declined to continue after batch {batch_index}",
                )
                self.report.write_incremental()
    ...
    def _run_batch(
        self, batch: list[InventoryAgent]
    ) -> list[tuple[InventoryAgent, str, Exception | None]]:
        ...
        with ThreadPoolExecutor(
            max_workers=self.options.parallelism,
            thread_name_prefix="te-agent-migrate",
        ) as executor:
            futures = [
                executor.submit(self._run_agent_captured, agent, correlation_id)
                for agent, correlation_id in jobs
            ]
        return [
            (agent, correlation_id, future.result())
            for (agent, correlation_id), future in zip(jobs, futures, strict=True)
        ]

    def _run_agent_captured(
        self, inventory_agent: InventoryAgent, correlation_id: str
    ) -> Exception | None:
        try:
    ...
    def _hard_stop_scope(self) -> str:
        if self.options.parallelism == 1:
            return "remaining agents were not touched"
        return (
            "agents in later batches were not touched; peers already running in the active "
    ...
    def _pause_after_failure(
        ...
        while True:
            ...
            try:
                ...
                target = self.api.match_agent(
                    self.api.list_agents(self.target_group.aid),
    ...
    def _run_agent(self, inventory_agent: InventoryAgent, correlation_id: str) -> None:
        with self.step(correlation_id, inventory_agent.hostname, "api_prevalidation"):
            ...
            self._require_target_name_available(
                target_agents,
                inventory_agent,
        ...
        if source is not None and source.online and target is not None:
            self._event(
                correlation_id,
                inventory_agent.hostname,
                "dual_online_detected",
                StepStatus.PAUSED,
                "No reset will run; the existing target will be validated before source deletion",
            )
        ...
        with self.ui_factory(inventory_agent) as ui:
            ...
            if target is None:
                if source is None or not source.online:
                    raise HumanDecisionRequired(
                        "No online source identity is available for the destructive migration"
                    )
                ...
                if not ui.is_reachable():
                    raise HardStop(
                        f"{inventory_agent.hostname} became unreachable after reset; "
                        f"{self._hard_stop_scope()}"
                    )
                self._event(
                    correlation_id,
                    inventory_agent.hostname,
                    "post_reset_reachability",
                    StepStatus.OK,
                    "HTTPS UI reachable",
                )
                with self.step(
                    correlation_id, inventory_agent.hostname, "ui_post_reset_authentication"
                ):
                    ui.authenticate()
                with self.step(correlation_id, inventory_agent.hostname, "ui_target_token"):
                    ui.set_account_group_token(
                        self.target_account_token,
                ...
                self._event(
                    ...
                    "identity_migration",
                    StepStatus.SKIPPED,
                    "Healthy target identity already exists; destructive UI actions skipped",
                )
            ...
            with self._test_lock:
                self._completed_target_agents[inventory_agent.hostname] = (
                    inventory_agent,
                    target,
                )
        ...
        *,
        ...
        try:
            with self.step(
                correlation_id,
                inventory_agent.hostname,
                "api_target_visibility",
                start_detail=(
                    f"polling destination API for up to {self.options.timeout_seconds} seconds"
                ),
            ):
                return self.api.wait_for_agent(
            ...
            while True:
                ...
                if decision == "recheck":
                    ...
                    self._event(
                        ...
                        "api_target_visibility_recheck",
                        StepStatus.PAUSED,
                        "One-shot read-only recheck still found no online target identity",
                    )
                    continue
                if decision == "skip":
                    raise AgentSkipped(
                        "Destination visibility remained indeterminate; "
                        "agent skipped without remediation"
                    ) from exc
                raise HumanDecisionRequired(
                    "Destination visibility remained indeterminate; "
    ...
    def _require_target_name_available(
        ...
        inventory_agent: InventoryAgent,
        *,
        current_target: AgentRecord | None,
    ) -> None:
        conflicts = [
            agent
            for agent in target_agents
            if agent.name.casefold() == inventory_agent.hostname.casefold()
            and (current_target is None or agent.agent_id != current_target.agent_id)
        ]
        if not conflicts:
            return
        summary = ", ".join(f"id={agent.agent_id} state={agent.state}" for agent in conflicts)
        raise HardStop(
            f"Destination display name {inventory_agent.hostname!r} is already owned by another "
            f"target identity ({summary}); no reset was attempted"
        )
    ...
    def _enforce_exact_target_name(
        self,
        target: AgentRecord,
        inventory_agent: InventoryAgent,
        correlation_id: str,
        ...
        try:
            if target.name != inventory_agent.hostname:
                with self.step(
                    correlation_id,
                    inventory_agent.hostname,
                    "api_target_exact_name",
                    start_detail=(
                        f"renaming destination identity {target.agent_id} from {target.name!r} to "
                        f"{inventory_agent.hostname!r}"
                    ),
                ):
                    self.api.update_agent_name(
                        self.target_group.aid,
                        target.agent_id,
                        inventory_agent.hostname,
                    )
            ...
            raise HardStop(
                f"Destination identity {target.agent_id} could not be renamed exactly to "
                f"{inventory_agent.hostname!r}; {self._hard_stop_scope()}"
            ) from exc
        ...
        try:
            with self.step(
                ...
                inventory_agent.hostname,
                "api_target_identity",
                start_detail="verifying target online status and exact target name",
            ):
                target = self.api.wait_for_agent_name(
                    self.target_group.aid,
                    agent_id=target.agent_id,
                    expected_name=inventory_agent.hostname,
                    timeout_seconds=self.options.timeout_seconds,
                    on_poll=lambda attempt, remaining: self.console.info(
                        inventory_agent.hostname,
                        f"exact-name poll {attempt}; {remaining:.0f}s remaining",
                    ),
                )
        except MigrationError as exc:
            raise HardStop(
                f"Destination identity {target.agent_id} did not become online with exact name "
                f"{inventory_agent.hostname!r}; {self._hard_stop_scope()}"
            ) from exc
    ...
    def _dry_run_agent(
        ...
        projected = target or AgentRecord(
            agent_id="<new-agent-id>",
            name=inventory_agent.hostname,
            hostname=inventory_agent.hostname,
            ip_addresses=[inventory_agent.ip_address],
            state="projected-online",
            aid=self.target_group.aid,
        )
        self.report.add_diff(
            {
                "agent": inventory_agent.hostname,
                "phase": "dry-run",
                ...
                "projected": {
                    "reset": "not executed",
                    "target_token": "not submitted",
                    "source_agent_deletion": (
                        "projected after target validation" if source is not None else "not needed"
                    ),
                    "automatic_registration_name": (f"{inventory_agent.hostname}-<new-local-id>"),
                    "name_normalization": "API v7 exact-name update projected",
                    "new_identity": asdict(projected),
                },
            }
        )
        ...
        self._event(
            ...
            "dry_run",
            StepStatus.OK,
            "All UI/API operations were read-only",
        )

    def _record_dry_run_test_readiness(
        self,
        ...
        data: dict[str, Any] = {
            ...
            "source_assigned_test_count": len(source_tests),
        }
        ...
        if self.options.strategy.value == "C":
            type_readiness = [
                {
                    "test_type": test_type,
                    "adapter_supported": test_type in API_TEST_TYPES,
                }
                for test_type in sorted({test.test_type for test in source_tests})
            ]
        ...
        self._event(
            correlation_id,
            inventory_agent.hostname,
            "dry_run_test_readiness",
            StepStatus.OK,
            detail,
            data,
        )

    def _prepare_strategy_c_source_test_inventory(
        ...
        if outside_inventory:
            summary = "; ".join(
                f"test {test_id}: {','.join(agent_ids)}"
                for test_id, agent_ids in sorted(outside_inventory.items())
            )
            raise HardStop(
                "Strategy C cannot migrate every source test because some tests are associated "
                f"with source agents outside the selected inventory ({summary})"
            )
        ...
        self.report.metadata.update(
            {
                "strategy_c_source_test_count": len(details),
                "strategy_c_unassigned_source_test_count": len(
                    self._strategy_c_unassigned_tests
                ),
    ...
    def _handle_strategy_c_unassigned_tests(
        self, inventory: list[InventoryAgent]
    ) -> None:
        ...
        if len(completed) != len(inventory):
            self._event(
                correlation_id,
                None,
                "strategy_c_unassigned_tests",
                StepStatus.SKIPPED,
                "Unassigned source tests retained because not every inventory agent completed",
            )
            return
        ...
        if self.options.mode is Mode.DRY_RUN:
                ...
                self.report.add_test(
                    {
                        ...
                        "action": "recreate-and-assign-all",
                        "status": "projected",
                        "target_test_id": None,
                        "target_test_name": test.name,
                        "detail": (
                            "Unassigned source test will be enabled and assigned to all "
                            "migrated agents"
                        ),
                        "source_aid": self.source_group.aid,
                        "target_aid": self.target_group.aid,
                ...
                self.report.add_stale_entry(
                    {
                        ...
                        "agent": "all-migrated-agents",
                        "source_test_id": test.test_id,
                        "source_test_name": test.name,
                        "test_type": test.test_type,
                        "source_aid": self.source_group.aid,
                        "policy": self.options.stale_policy.value,
                        "recreated_in_target": True,
            ...
            self._event(
                ...
                StepStatus.CHANGED,
                (
                    f"projected {len(self._strategy_c_unassigned_tests)} unassigned test(s) "
                    f"across {len(completed)} migrated agent(s)"
                ),
            )
            return
        ...
        self._event(
            ...
            StepStatus.CHANGED,
            (
                f"migrated {len(self._strategy_c_unassigned_tests)} unassigned test(s) "
                f"and assigned each to {len(completed)} target agent(s)"
            ),
        )
            ...
            for test_id in test_ids:
                ...
                if test is None:
                    self.report.add_unsupported_test(
                        {
                            "test_id": test_id,
                            "reason": "Assigned test not visible in Strategy C source snapshot",
                        }
                    )
                    continue
    ...
    def _handle_tests(
        ...
        inventory_agent: InventoryAgent,
        *,
        target_tests: list[TestRecord] | None = None,
        dry_run: bool = False,
        source_agent_id: str | None = None,
    ) -> None:
        with self._test_lock:
            self._handle_tests_locked(
                tests,
                target,
                correlation_id,
                inventory_agent,
            ...
            if self.options.strategy.value == "C":
                if (
                    ...
                    with self.step(
                        correlation_id,
                        inventory_agent.hostname,
                        "api_target_test_ready",
                        start_detail=(
                            f"verifying target test {action.target_test_id} is enabled and "
                            f"assigned to agent {target.agent_id}"
                        ),
                    ):
                        self.api.wait_for_test_ready(
                            self.target_group.aid,
                            test_type=action.test_type,
                            test_id=action.target_test_id,
                            agent_id=target.agent_id,
                            ...
                            on_poll=lambda attempt, remaining: self.console.info(
                                inventory_agent.hostname,
                                f"target-test poll {attempt}; {remaining:.0f}s remaining",
                            ),
                        )
                self._record_source_test_policy(
                    test,
    ...
    def _record_source_test_policy(
        self,
        test: TestRecord,
        action_status: str,
        inventory_agent: InventoryAgent,
        ...
        self.report.add_stale_entry(
            {
                ...
                "source_aid": self.source_group.aid,
                "policy": self.options.stale_policy.value,
                "recreated_in_target": successfully_recreated,
                "action": action,
            }
        )

    def _cleanup_source_tests(self) -> None:
        correlation_id = str(uuid.uuid4())
        if self.options.strategy.value != "C":
            self._event(
                correlation_id,
                ...
                "source_test_cleanup",
                StepStatus.SKIPPED,
                f"Not applicable to Strategy {self.options.strategy.value}",
            )
            return
        if self.options.stale_policy.value == "keep":
            self._event(
                ...
                "source_test_cleanup",
                StepStatus.SKIPPED,
                "Strategy C keep policy retained source tests",
            )
            return
        if not self._source_test_cleanup_candidates:
            self._event(
                ...
                "source_test_cleanup",
                StepStatus.SKIPPED,
                "No successfully recreated source tests were eligible for deletion",
            )
            return
        for test in self._source_test_cleanup_candidates.values():
                ...
                self.report.add_stale_entry(
                    {
                        ...
                        "source_aid": self.source_group.aid,
                        "policy": "remove",
                        "action": "kept-incomplete-associations",
                        "recreated_in_target": True,
                        "missing_source_agent_ids": missing_source_agent_ids,
                    }
                )
                self._event(
                    correlation_id,
                    ...
                    "source_test_cleanup",
                    StepStatus.SKIPPED,
                    f"Source test {test.test_id} retained because associated source agents "
                    f"did not complete: {', '.join(missing_source_agent_ids)}",
                )
                continue
            with self.step(
                correlation_id,
                None,
                "delete_source_test",
                start_detail=(f"deleting recreated source test {test.test_id} ({test.test_type})"),
            ):
            ...
            with self.step(
                ...
                "source_test_absent",
                start_detail=(
                    f"verifying source test {test.test_id} is absent for up to "
                    f"{self.options.timeout_seconds} seconds"
                ),
            ):
                self.api.wait_for_test_absent(
                    self.source_group.aid,
                    test_id=test.test_id,
                    timeout_seconds=self.options.timeout_seconds,
                    on_poll=lambda attempt, remaining: self.console.info(
                        "global",
                        f"source-test absence poll {attempt}; {remaining:.0f}s remaining",
                    ),
                )
            self.report.add_stale_entry(
                {
    ...
    def _ensure_destructive_confirmation(self) -> None:
    ...
    def _record_agent(
        self, phase: str, inventory_agent: InventoryAgent, agent: AgentRecord | None
    ) -> None:
        if agent is None:
            return
        self.report.add_agent(
            {
                ...
                "aid": agent.aid,
                "account_group": agent.account_group_name,
                "labels": agent.labels,
                "state": agent.state,
                "test_ids": agent.test_ids,
            }
        )

    def _record_diff(
        self, inventory_agent: InventoryAgent, before: LocalSnapshot, after: LocalSnapshot
    ) -> None:
        self.report.add_diff(
            {
                "agent": inventory_agent.hostname,
                ...
                "changes": recursive_diff(before.state, after.state),
            }
        )
    ...
    def _verify_identity_metadata(
        ...
        self._event(
            ...
            inventory_agent.hostname,
            "identity_metadata",
            StepStatus.OK if preserved_name and preserved_labels else StepStatus.CHANGED,
            detail,
        )

    @staticmethod
    def _test_row(phase: str, test: TestRecord, source: AgentRecord | None) -> dict[str, Any]:
        return {
            "phase": phase,
            "test_id": test.test_id,
            "name": test.name,
            ...
            "bound_agents": test.agents,
            "enabled": test.enabled,
            "alert_rules": test.alert_rules,
            "source_agent_id": None if source is None else source.agent_id,
        }
